    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                  <div className="flex items-start space-x-3.5">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xs font-semibold px-2 py-0.5 rounded bg-[#0F172A] border border-slate-700 text-slate-200">
                          {ex.trade.security.ticker}
                        </span>
                        <span className="text-xs font-mono font-bold text-emerald-400">
                          ${(ex.trade.tradeValue / 1000000).toFixed(1)}M
                        </span>
                        {isTopShowcase && (
                      ...
                      <div className="text-xs text-slate-300 mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
                        <span className="flex items-center gap-1 text-slate-200">
                          ...
                          {ex.trade.counterparty.name} ({ex.trade.counterparty.id})
                        </span>
                        <span className="text-rose-400 font-semibold font-mono text-[11px]">
                          • {ex.trade.counterparty.priorFailures} prior failures
                        </span>
                      </div>
                  ...
                  <div className="flex items-center justify-between md:flex-col md:items-end gap-2 shrink-0 border-t md:border-t-0 pt-2 md:pt-0 border-slate-700">
                    <div className="flex items-center space-x-1.5 text-xs font-mono text-amber-300">
                      ...
                      <span>Cutoff: {Math.floor(ex.trade.cutoffMinutesRemaining / 60)}h {ex.trade.cutoffMinutesRemaining % 60}m</span>
                    </div>
                    ...
                    <button
                      ...
                      className="flex items-center space-x-1.5 px-3.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold shadow transition-all"
                    >
        ...
        <div className="space-y-6">
          {/* Counterparty Fail Concentration */}
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={COUNTERPARTY_FAIL_DISTRIBUTION} layout="vertical" margin={{ left: 5, right: 10, top: 5, bottom: 5 }}>
                  ...
                  <Tooltip 
                    ...
                    formatter={(value: any) => [`${value} Settlement Failures`, 'Failures']}
                  />
                  <Bar dataKey="fails" fill="#FF3B5C" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>

            <div className="p-2.5 rounded-lg bg-[#162032] border border-slate-700 text-[11px] text-slate-300 flex items-start gap-2">
              <Zap className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
              <span>
                <strong className="text-white">CP-192 (Apex Prime)</strong> accounts for repeated failure risk. Autonomous escalation playbook active.
              </span>
            </div>
          </div>
          ...
          <div className="bg-[#0F172A] border border-slate-700/80 p-5 rounded-2xl space-y-3 shadow-lg">
            <div className="flex items-center justify-between border-b border-slate-800 pb-2">
              ...
              <span className="text-[10px] font-mono text-cyan-400 font-bold">Snowflake KB</span>
            </div>
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="p-2.5 rounded-lg bg-[#162032] border border-slate-700">
                <div className="flex items-center justify-between text-slate-200 font-medium">
                  ...
                  <span className="text-emerald-400 font-mono font-bold">100% On-Time</span>
                </div>
                <div className="text-[11px] text-slate-400 mt-1">
                  Auto-adjustment protocol authorized for variances under $5,000 threshold.
                </div>
              </div>
            </div>
          </div>
