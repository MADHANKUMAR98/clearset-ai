import type { IRiskService } from './types';
import type { RiskFactor, RiskScoreBreakdown, Trade, ExceptionSeverity } from '../types';

export class LocalRiskService implements IRiskService {
  public calculateRisk(trade: Trade): RiskScoreBreakdown {
    ...
    // 3. Trade Exposure / Value (+20 for > $1.0M, +10 for > $500k)
    if (trade.tradeValue >= 1000000) {
      factors.push({
        ...
        points: 20,
        severity: 'HIGH',
        explanation: `Gross exposure of $${(trade.tradeValue / 1000000).toFixed(1)}M exceeds High-Value Operations Threshold ($1.0M). CSDR penalty risk elevated.`,
        iconName: 'DollarSign',
      });
      totalScore += 20;
...
export const riskService: IRiskService = new LocalRiskService();
